...
func runVerify(cmd *cobra.Command, args []string) error {
	...
	pods, err := k8sClient.GetPods(ctx, namespace, "app.kubernetes.io/name=inferencehub")
	...
	uiInstance.PrintKeyValue("Total Pods", fmt.Sprintf("%d", len(pods.Items)))
	...
	if readyCount == len(pods.Items) {
		...
		uiInstance.Warn("Some pods are not ready (%d/%d)", readyCount, len(pods.Items))
